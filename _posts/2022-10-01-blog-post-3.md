...
===
<figure>
<img src='/images/transformers.png' height="500">
...
The layer normalization normalize values in each layer to have 0 mean and unit variance for each hidden unit $h_{i}$ computed as: $ h_{i} = \frac{g x (h_{i} - \mu)}{\sigma} $ where g is a variable and $ \mu = \frac{1}{H} \sum_{i=1}^{H}h_{i}$ and $ \sigma = \sqrt{\frac{1 x \sum_{i=1}^{H} (h_{i} - \mu)^{2}}{H}} $
